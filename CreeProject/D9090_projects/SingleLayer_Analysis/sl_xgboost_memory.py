...
class Profiling_Model:
    ...
    Synopsis
    import sl_xgboost_memory
    def run_it();

    Description
    This is the main class of module sl_xgboost_memory. This class contains everything we need to run this model.
    ...
    def load_data(self):
        ...
        tmp = self.raw_df.copy()
        #test = tmp[4:self.layer_points:10]
        #self.validation = tmp[-tmp.isin(test)].dropna().reset_index(drop=True)
        #self.test_set = test.reset_index(drop=True)
        self.validation = tmp
    ...
    def train_model(self,random_state=1,frac=0.35,n_estimators=400,max_depth=8):
        ...
        tmp_df=self.raw_df.sample(frac=frac,random_state=random_state)
    ...
    def test_model(self):
        ...
        self.test_evaluate(test_y.values.ravel(),pred2)
        #self.xx=test_X
        #self.y1=test_y.values.ravel()
        #self.y2=pred2
        self.Smith_2lines(xx=np.array(test_X.iloc[:,2:4]),y1=np.array(test_y.values.ravel()),y2=np.array(pred2),name0=self.tmp_layer+1)
